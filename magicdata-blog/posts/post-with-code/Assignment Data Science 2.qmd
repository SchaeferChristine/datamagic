...
```{r}
d_long %>% 
  get_sentiment(tweet, method = "custom", lexicon = insults)
```
...
```{r}
set.seed(42)
folds <- vfold_cv(d_train, v = 5, repeats = 1)
```


## get_sentiment

Because R does not tolerate that being made in the recipe today. Two AIs are clueless why and so am I. So I'll try it the long way. I'll compute the sentiment analysis outside of the recipe and then import the result into it as a new predictor column.

```{r}
n_insults <- train2 %>% syuzhet::get_sentiment(tweet, method = "custom", lexicon = insults) %>% 
  as_tibble()
```

```{r}
n_senti <- train2 %>% get_sentiment(tweet, method = "custom", lexicon = afinn)
```
...
```{r}
rec1 <- 
  recipe(class ~ ., data = train2) %>% 
  update_role(id, new_role = "id")  %>% 
  #recipes::step_mutate(n_insult = syuzhet::get_sentiment(tweet, method = "custom", lexicon = insults)) %>%  ERROR, could not find lexicon
  #step_mutate(rsenti = syuzhet::get_sentiment(tweet, method = "custom", lexicon = afinn)) %>% ERROR, could not find lexicon
  step_tokenize(tweet) %>%
  step_stopwords(tweet, keep = FALSE, custom_stopword_source = stop2) %>%
  #step_word_embeddings(tweet, embeddings = wiki, aggregation = "mean") %>%  
  step_zv() %>% 
